...
class CustomizationPage(tk.Frame):
    def __init__(self, master, item_name, base_price):
        ...
        self.var_size = tk.StringVar(value="Small")
        ttk.Label(self, text="Select Size:").pack()
        ttk.Combobox(self, textvariable=self.var_size, values=["Small", "Regular"]).pack()
...
class CustomerInfoPage(tk.Frame):
    ...
    def save_info(self):
        for key, entry in self.entries.items():
            self.master.customer_info[key] = entry.get()
...
class PaymentInfoPage(tk.Frame):
    ...
    def save_info(self):
        for key, entry in self.entries.items():
            self.master.payment_info[key] = entry.get()
